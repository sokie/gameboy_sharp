...
namespace CpuTestHarness
{
    ...
    public sealed class OpcodeResult
    {
        public string Name = "";
        public int Total;
        public int Passed;
        public int CycleMismatches;
        public int ImeMismatches;
        public readonly List<string> Failures = new(); // a few detailed diffs for the report
    ...
    /// instruction over a flat memory bus, and the resulting registers and RAM are compared
    /// against the vector's expected final state. This is the definitive way to prove every
    /// opcode (and its flag behavior) is implemented correctly.
    /// </summary>
    public sealed class Sm83TestRunner
    {
        ...
        private bool RunOneTest(JsonElement test, OpcodeResult result)
        {
            ...
            // Informational only (not part of pass/fail): cycle count and IME.
            int expectedTCycles = test.GetProperty("cycles").GetArrayLength() * 4;
            if (tCycles != expectedTCycles) result.CycleMismatches++;
            if (final.TryGetProperty("ime", out JsonElement finalIme) &&
                (cpu.InterruptMasterEnable ? 1 : 0) != finalIme.GetInt32())
            {
                result.ImeMismatches++;
            }
